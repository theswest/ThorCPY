...
DISPLAY_INIT_DELAY = 1.2
SCRCPY_CREATION_DELAY = 0.3
SCRCPY_RETRY_DELAY = 0.7
        ...
        for processName, process in enumerate(self.processes):
            try:
                if process.poll() is not None:
                    logger.warning(f"Process {processName} (PID: {process.pid}) is no longer alive (exit code: {process.poll()})")
                    return process
